...
def generate_research_ideas(
    ...
    if user_interests.strip():
        user_interests_block = (
            f"RESEARCHER PROFILE (hard constraint — at least 2 ideas MUST connect to this):\n"
            f"The researcher's interests are: {user_interests.strip()}"
        )
    else:
        user_interests_block = (
            "RESEARCHER PROFILE: None provided. "
            "Generate ideas spanning different technical approaches and application domains."
        )

    prompt = BASE_IDEA_PROMPT.format(
        title=metadata["title"],
        summary=metadata["summary"],
        user_interests_block=user_interests_block,
    )
